...
class Solution:
    def addOneRow(self, root: TreeNode, v: int, d: int) -> TreeNode:
        ...
        def traverse(pNode: TreeNode, cNode: TreeNode, currDepth: int, isLeft: bool):

            global gRoot

            if currDepth == d:
                if not pNode:
                    gRoot = TreeNode(v, gRoot, None)
                elif isLeft:
                    pNode.left = TreeNode(v, cNode, None)
                else:
                    pNode.right = TreeNode(v, None, cNode)
            else:
                if cNode:
                    traverse(cNode, cNode.left, currDepth+1, True)
                    traverse(cNode, cNode.right, currDepth+1, False)

        traverse(None, gRoot, 1, True)
