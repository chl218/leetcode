...
For a given query word, the spell checker handles two categories of spelling mistakes:
    ...
    Vowel Errors: If after replacing the vowels ('a', 'e', 'i', 'o', 'u') of the query word with any vowel individually,
                  it matches a word in the wordlist (case-insensitive), then the query word is returned with the same
    ...
    def spellchecker(self, wordlist: List[str], queries: List[str]) -> List[str]:
        ...
        for query in queries:
            ...
            if queryL in words_cap:
                res.append(words_cap[queryL])
                continue

            queryLV = devowel(queryL)
            if queryLV in words_vow:
                res.append(words_vow[queryLV])
                continue
