...
class Solution:
    def shortestToChar(self, s: str, c: str) -> List[int]:
        ...
        def getCounts(s: str, c: str, start: int, stop: int, step: int):
            stack = []
            counts = []
        ...
        rcount = getCounts(s, c, 0, len(s), 1)
        lcount = getCounts(s, c, len(s) - 1, -1, -1)
        lcount.reverse()

        counts = []
        for i in range(len(s)):
            if rcount[i] < lcount[i]:
                counts.append(rcount[i])
            else:
                counts.append(lcount[i])
