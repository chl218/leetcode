    ...
    void connect(TreeLinkNode *root) {
        ...
        while(!lst.empty()) {
            ...
            for(int i = 0; i < size; i++) {
                TreeLinkNode *curr_node = lst.back(); lst.pop_back();
                
                curr_node->next = next_node;
                
                if(curr_node->right) lst.push_front(curr_node->right);
                if(curr_node->left) lst.push_front(curr_node->left);
